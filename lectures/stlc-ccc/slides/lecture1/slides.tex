...
% Add a top-rule to inference axioms.
\renewcommand{\infax}[2][]{\infrule[#1]{}{#2}}
...
\newcommand{\sing}[2]{\s{\v{#1}\mapsto \t{#2}}}
...
\begin{frame}{Substitutions}
  ...
  \emph{NB.} The hygiene condition
  $\v{x} \notin \fv(\s{\sigma}(\mathcal{V}))$ is necessary to avoid
  \emph{variable capture}.  It can be avoided through systematic
  $\alpha$-renaming (no ``shadowing'').
\end{frame}
...
\begin{frame}{Substitution equality (2/2)%
  ...
  \infrule[SEEmpty]
  {\sub{\Gamma}{\sigma}{\cempty}}
  {\seq{\Gamma}{\sigma}{\sempty}{\cempty}}
...
\begin{frame}{Getting rid of indices: use just $\vz$ + weakening}
  ...
  \begin{multicols}{2}
    ...
    \begin{align*}
      ...
      &\; \; \vdots
    \end{align*}}%
    \onslide<2->{\typicallabel{Weaken}\infax[Weaken]
      {\sub{\Gamma \cext \bd{x}{A}}{\wk}{\Gamma}}\typicallabel{}}
  \end{multicols}
...
\renewcommand{\tp}[3]{#1 \vdash #3}
\renewcommand{\sub}[3]{#1 \vdash #3}

\begin{frame}{Intuitionistic Propositional Logic \hfill \fbox{$\tp{\Gamma}{t}{A}$}}
  \begin{multicols}{2}
    \typingrules
  \end{multicols}
\end{frame}
